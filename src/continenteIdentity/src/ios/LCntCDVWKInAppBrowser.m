...
#import "LCntCDVWKInAppBrowser.h"
...
#pragma mark LCntCDVWKInAppBrowser

@interface LCntCDVWKInAppBrowser () {
    NSInteger _previousStatusBarStyle;
}
@end

@implementation LCntCDVWKInAppBrowser

static LCntCDVWKInAppBrowser* instance = nil;
...
- (void)open:(CDVInvokedUrlCommand*)command
{
    ...
    NSString* clientId = [command argumentAtIndex:3 withDefault:@"" andClass:[NSString class]];
    NSString* clientSecret = [command argumentAtIndex:4 withDefault:@"" andClass:[NSString class]];
    NSString* encodedClientCredentials = [self encodeClientCredentials:clientId withClientSecret:clientSecret];
    ...
    if (url != nil) {
        ...
        if ([target isEqualToString:kInAppBrowserTargetSelf]) {
            ...
            [self openInInAppBrowser:absoluteUrl withOptions:options withAuthorizationHeaderValue:encodedClientCredentials];
        }
...
- (NSString*)encodeClientCredentials:(NSString*)clientId withClientSecret:(NSString*)clientSecret
{
    ...
    NSData * credentialsEncoded = [credentials dataUsingEncoding:NSUTF8StringEncoding];

    NSString * authorizationHeaderValue = [NSString stringWithFormat:@"Basic %@", [credentialsEncoded base64EncodedStringWithOptions:0]];

    return authorizationHeaderValue;
}

- (void)openInInAppBrowser:(NSURL*)url withOptions:(NSString*)options withAuthorizationHeaderValue:(NSString*)authorizationHeaderValue
{
    NSLog(@"openInInAppBrower : ");
    NSLog(authorizationHeaderValue);
    LCntCDVInAppBrowserOptions* browserOptions = [LCntCDVInAppBrowserOptions parseOptions:options];
    ...
    if (self.inAppBrowserViewController == nil) {
        self.inAppBrowserViewController = [[LCntCDVWKInAppBrowserViewController alloc] initWithBrowserOptions: browserOptions andSettings:self.commandDelegate.settings];
    ...
    [self.inAppBrowserViewController navigateTo:url withAuthorizationHeaderValue:authorizationHeaderValue];
...
- (void)show:(CDVInvokedUrlCommand*)command withNoAnimate:(BOOL)noAnimate
{
    ...
    __block LCntCDVInAppBrowserNavigationController* nav = [[LCntCDVInAppBrowserNavigationController alloc]
                                                        initWithRootViewController:self.inAppBrowserViewController];
    nav.orientationDelegate = self.inAppBrowserViewController;
    nav.navigationBarHidden = YES;
    nav.modalPresentationStyle = self.inAppBrowserViewController.modalPresentationStyle;
    
    __weak LCntCDVWKInAppBrowser* weakSelf = self;
...
{
    // Ensure a message handler bridge is created to communicate with the LCntCDVWKInAppBrowserViewController
...
@end //LCntCDVWKInAppBrowser

#pragma mark LCntCDVWKInAppBrowserViewController

@implementation LCntCDVWKInAppBrowserViewController
...
- (id)initWithBrowserOptions: (LCntCDVInAppBrowserOptions*) browserOptions andSettings:(NSDictionary *)settings
{
    self = [super init];
    if (self != nil) {
        _browserOptions = browserOptions;
        _settings = settings;
        self.webViewUIDelegate = [[LCntCDVWKInAppBrowserUIDelegate alloc] initWithTitle:[[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleDisplayName"]];
...
- (void)navigateTo:(NSURL*)url withAuthorizationHeaderValue:(NSString*)authorizationHeaderValue
{
    NSLog(@"navigateTo : ");
    NSLog(authorizationHeaderValue);
    NSMutableURLRequest* request = [NSMutableURLRequest requestWithURL:url];
    // if ([authorizationHeaderValue length] > 0)
    // {
...
@end //LCntCDVWKInAppBrowserViewController
